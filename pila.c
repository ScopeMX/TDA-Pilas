...
//Funci ï ¿½ï¿½ï¿½ï¿½n que mete elementos a la pila
int
...
int
almacenar_documento_html(FILE * archivo_html, Pila * A)
{
	iniciar_etiquetas();
...
guardar(Pila * A)
{
	...
	error = push(A, identificador_etiqueta);
	...
	if (unica == 1 || unica == 2 || unica == 3) {
		error = push(A, identificador_etiqueta * -1);
	}
	reiniciar_variables();
	return 0;
}
...
int
validar_documento_html(Pila * A)
{
	int		validar_suma = 0;
	//debe ser 0 al final para que el documento se considere correcto
		int		posicion = 0;
	for (posicion = 0; posicion < A->ultimo_agregado; posicion++) {
		validar_suma = validar_suma + A->elementos_pila[posicion];
	}
	if (validar_suma == 0) {
		return 1;
	} else {
